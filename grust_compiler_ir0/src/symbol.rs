...
pub enum SymbolKind {
    ...
    Identifier {
        ...
        /// Is it generated by the compiler?
        codegen: bool,
    },
    /// Initialization kind.
    Init {
...
impl Table {
    ...
    /// Insert generated identifier in symbol table.
    pub fn insert_generated_ident(
        &mut self,
        name: Ident,
        typing: Option<Typ>,
        local: bool,
    ...
    /// Tell wether the id was generated.
    pub fn is_codegen(&self, id: usize) -> bool {
        let symbol = self
            .get_symbol(id)
            .unwrap_or_else(|| panic!("expect symbol for {id}"));
        match symbol.kind() {
            SymbolKind::Identifier { codegen, .. } => *codegen,
            _ => noErrorDesc!(),
        }
    }

    /// Set that id was generated.
    pub fn set_codegen(&mut self, id: usize) {
        let symbol = self
            .get_symbol_mut(id)
            .unwrap_or_else(|| panic!("expect symbol for {id}"));
        match symbol.kind_mut() {
            SymbolKind::Identifier { codegen, .. } => *codegen = true,
            _ => noErrorDesc!(),
        }
    }
