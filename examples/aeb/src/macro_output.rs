...
pub mod toto_service {
    ...
    impl TotoServiceTimer {
        pub fn get_timer(&self) -> i64 {
            match self {
                T::timeout_fresh_ident => 2000u64 as i64,
            }
        }
    }
            ...
            loop {
                tokio::select! {
                    input = input.next() => if let Some(input) = input
                    {
                        match input
                        {
                            I :: speed_km_h(speed_km_h, timestamp) =>
                            service.handle_speed_km_h(timestamp, speed_km_h).await, I ::
                            pedestrian_l(pedestrian_l, timestamp) =>
                            service.handle_pedestrian_l(timestamp, pedestrian_l).await,
                            I :: pedestrian_r(pedestrian_r, timestamp) =>
                            service.handle_pedestrian_r(timestamp, pedestrian_r).await,
                            I :: timer(T :: timeout_fresh_ident, timestamp) =>
                            service.handle_timeout_fresh_ident(timestamp).await,
                        }
                    } else { break ; }
                }
            }
