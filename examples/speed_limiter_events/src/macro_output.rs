            ...
            loop {
                tokio::select! {
                    input = input.next() => if let Some(input) = input
                    {
                        match input
                        {
                            I :: activation(activation, timestamp) =>
                            service.handle_activation(timestamp, activation).await, I ::
                            set_speed(set_speed, timestamp) =>
                            service.handle_set_speed(timestamp, set_speed).await, I ::
                            speed(speed, timestamp) =>
                            service.handle_speed(timestamp, speed).await, I ::
                            vacuum_brake(vacuum_brake, timestamp) =>
                            service.handle_vacuum_brake(timestamp, vacuum_brake).await,
                            I :: kickdown(kickdown, timestamp) =>
                            service.handle_kickdown(timestamp, kickdown).await, I ::
                            failure(failure, timestamp) =>
                            service.handle_failure(timestamp, failure).await, I ::
                            vdc(vdc, timestamp) =>
                            service.handle_vdc(timestamp, vdc).await, I ::
                            timer(T :: period_fresh_ident, timestamp) =>
                            service.handle_period_fresh_ident(timestamp).await,
                        }
                    } else { break ; }
                }
            }
