    ...
    pub async fn run_loop(self, input: impl futures::Stream<Item = TotoServiceInput>) {
        ...
        loop {
            tokio::select! {
                ...
                {
                    TotoServiceInput :: activation(activation) =>
                    service.handle_activation(activation).await,
                    TotoServiceInput :: set_speed(set_speed) =>
                    service.handle_set_speed(set_speed).await, TotoServiceInput
                    :: speed(speed) => service.handle_speed(speed).await,
                    TotoServiceInput :: vacuum_brake(vacuum_brake) =>
                    service.handle_vacuum_brake(vacuum_brake).await,
                    TotoServiceInput :: kickdown(kickdown) =>
                    service.handle_kickdown(kickdown).await, TotoServiceInput ::
                    failure(failure) => service.handle_failure(failure).await,
                    TotoServiceInput :: vdc(vdc) =>
                    service.handle_vdc(vdc).await
