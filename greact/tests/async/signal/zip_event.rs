use std::time::Duration;

use colored::Colorize;
use futures::StreamExt;
use greact::{
    event::input_channel as event_input_channel,
    signal::{input_channel, zip_event, Signal},
};
use tokio::{sync::mpsc::channel, time::sleep};

struct MainInput {
    a: Option<i64>,
    b: i64,
}
struct MainState {
    mem: i64,
}
impl MainState {
    fn init() -> Self {
        Self { mem: 0 }
    }
    fn step(&mut self, input: MainInput) -> i64 {
        let o = match input.a {
            Some(a) => a + input.b + self.mem,
            None => input.b + self.mem,
        };
        self.mem = o;
        o
    }
}

/// ```GRRust
/// component main(a: int?, b: int) {
///     mem: int = 0 fby o;
/// 	out o: int = when a then a + b + mem else b + mem;
/// }
///
/// interface {
///     event int i; // import sdv.adas.fusion.i1;
///     signal int j; // import sdv.adas.fusion.i2;
///     signal int o; // import sdv.adas.fusion.o;
///
///     stream int res = signal_sample(i, j);
///
///     o = signal_emit(res);
/// }
/// ```
#[tokio::test]
async fn main() -> Result<(), String> {
    // signals management
    let (tx_i, i) = event_input_channel();
    let (tx_j, j) = input_channel(0);
    let (tx_o, mut rx_o) = channel::<i64>(1);
    let inputs = zip_event(i, j); // construct the ZipEventSignal structure

    // synchronous state-machine initialisation
    let mut state = MainState::init();

    // input/output management pushed in another thread
    tokio::spawn(async move {
        let mut n = 0;
        sleep(Duration::from_millis(1)).await;
        loop {
            println!("{}", format!("emit i: {n}").green());
            if let Err(_) = tx_i.send(n).await {
                println!("input receiver dropped");
                return;
            }
            n += 1;
            sleep(Duration::from_millis(400)).await;
        }
    });
    tokio::spawn(async move {
        let mut n = 0;
        sleep(Duration::from_millis(1)).await;
        loop {
            println!("{}", format!("emit j: {n}").green());
            if let Err(_) = tx_j.send(n).await {
                println!("input receiver dropped");
                return;
            }
            n += 10;
            sleep(Duration::from_millis(200)).await;
        }
    });
    tokio::spawn(async move {
        loop {
            let n = rx_o.recv().await.expect("should not end");
            println!("{}", format!("output received: {n}").yellow());
        }
    });

    // synchronous state-machine launched
    let inputs = inputs.push();
    tokio::pin!(inputs);
    let run = crate::Run::new();
    loop {
        if run.should_stop() {
            break;
        }
        let (a, b) = inputs.next().await.expect("should not end");
        println!("{}", format!("input received: {:?}", (a, b)).red());
        let b = state.step(MainInput { a, b });
        println!("{}", format!("compute: {b}").blue());
        if let Err(e) = tx_o.send(b).await {
            return Err(format!("output receiver dropped ({e})"));
        }
    }

    Ok(())
}
