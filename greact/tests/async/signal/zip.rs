...
use greact::signal::{input_channel, map, zip, Signal};
...
impl MainState {
    ...
    fn step(&mut self, input: MainInput) -> i64 {
        let o = input.a.0 + input.a.1 + self.mem;
        self.mem = o;
        o
    }
}
...
/// ```GRRust
/// component main(a: (int, int)) {
///     mem: int = 0 fby o;
/// 	out o: int = a.0 + a.1 + mem;
/// }
///
/// interface {
...
///     signal int t = zip(j, e);
///
///     stream int a = signal_sample(t);
...
async fn main() {
    ...
    let t = zip(j, e); // construct the ZipSignal structure
    ...
    tokio::pin!(t);
    loop {
        let a = t.next().await.expect("should not end");
