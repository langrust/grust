...
pub fn prioritize(alarm: Alarm, other: Alarm) -> i64 {
    ...
    let priority_ordering = Expr::FunctionCall(
        parse_quote! {
            priority_order(alarm.priority, other.priority)
        },
    );
    if raise_difference {
...
pub fn sorting(alarms: [Alarm; 10usize]) -> [Alarm; 10usize] {
    alarms.sort(prioritize)
}
