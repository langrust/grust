...
impl MainState {
    ...
    pub fn step(&'static mut self, input: SystemInput) -> SystemOutput {
        // put everything in parallel without scheduling it
        RUNTIME.block_on(async {
            println!("step!");

            let _ = futures::join!(
                async { self.rgb_images.send(input.rgb_images).await.unwrap() },
                async { self.point_cloud.send(input.point_cloud).await.unwrap() },
                async { self.distances.send(input.distances).await.unwrap() },
            ...
            SystemOutput {
                moving_objects: self.output.recv().await.unwrap(),
            }
        })
    }
}
