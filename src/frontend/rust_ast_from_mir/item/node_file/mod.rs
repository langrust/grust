...
use self::import::rust_ast_from_mir as import_rust_ast_from_mir;
...
pub fn rust_ast_from_mir(node_file: NodeFile) -> File {
    let mut items = node_file
        .imports
        .into_iter()
        .map(|import| Item::Import(import_rust_ast_from_mir(import)))
        .collect::<Vec<_>>();
    let input_structure = input_rust_ast_from_mir(node_file.input);
    let (state_structure, state_implementation) = state_rust_ast_from_mir(node_file.state);
