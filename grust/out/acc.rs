    ...
    pub mod adaptive_cruise_control_service {
        ...
        impl AdaptiveCruiseControlService {
            ...
            pub async fn handle_init(
                ...
                let ActivateOutput { c: cond } = <ActivateState as grust::core::Component>::step(
                    &mut self.activate,
            ...
            pub async fn handle_radar_m(
                ...
                if self.delayed {
                    ...
                    if self.context.radar_m.is_new() || self.context.x.is_new() {
                        let grust :: std :: time :: derivation :: DeriveOutput {d : vel_delta} = < grust :: std :: time :: derivation :: DeriveState as grust :: core :: Component > :: step (& mut self . derive , grust :: std :: time :: derivation :: DeriveInput {x : radar_m , t : x}) ;
                        self.context.vel_delta.set(vel_delta);
                    }
            ...
            pub async fn handle_delay_adaptive_cruise_control(
                ...
                if self.input_store.not_empty() {
                    ...
                    *acc_active_ref = _acc_active_input_store.map(|(x, _)| x);
                    ...
                    self.context.x.set(x);
                    if self.context.radar_m.is_new() || self.context.x.is_new() {
                        let grust :: std :: time :: derivation :: DeriveOutput {d : vel_delta} = < grust :: std :: time :: derivation :: DeriveState as grust :: core :: Component > :: step (& mut self . derive , grust :: std :: time :: derivation :: DeriveInput {x : self . context . radar_m . get () , t : x}) ;
                        self.context.vel_delta.set(vel_delta);
                    }
                    if self.context.cond.is_new()
                        || self.context.radar_m.is_new()
