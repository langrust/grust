...
pub mod runtime {
    ...
    impl Runtime {
        ...
        pub async fn run_loop(
            ...
            }
            {
                let res = runtime
                    .timer
                    .send((T::PeriodInRegulation, init_instant))
                    .await;
                if res.is_err() {
                    return;
                }
    ...
    pub mod speed_limiter_service {
        ...
        impl SpeedLimiterService {
            ...
            pub async fn handle_period_in_regulation(&mut self, instant: std::time::Instant) {
                ...
                {
                    let res = self.timer.send((T::PeriodInRegulation, instant)).await;
                    if res.is_err() {
                        return;
                    }
                }
