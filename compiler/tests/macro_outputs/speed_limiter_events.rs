...
pub mod toto_service {
    ...
    impl TotoServiceTimer {
        pub fn get_timer(&self) -> i64 {
            match self {
                ...
                T::period_fresh_ident_1 => 10u64 as i64,
            }
        }
    }
    ...
    impl TotoService {
        ...
        pub async fn run_loop(self, input: impl futures::Stream<Item = I>) {
            ...
            loop {
                tokio::select! { input = input . next () => if let Some (input) = input { match input { I :: activation (activation , timestamp) => service . handle_activation (timestamp , activation) . await , I :: set_speed (set_speed , timestamp) => service . handle_set_speed (timestamp , set_speed) . await , I :: speed (speed , timestamp) => service . handle_speed (timestamp , speed) . await , I :: vacuum_brake (vacuum_brake , timestamp) => service . handle_vacuum_brake (timestamp , vacuum_brake) . await , I :: kickdown (kickdown , timestamp) => service . handle_kickdown (timestamp , kickdown) . await , I :: failure (failure , timestamp) => service . handle_failure (timestamp , failure) . await , I :: vdc (vdc , timestamp) => service . handle_vdc (timestamp , vdc) . await , I :: timer (T :: period_fresh_ident , timestamp) => service . handle_period_fresh_ident (timestamp) . await , , I :: timer (T :: period_fresh_ident_1 , timestamp) => service . handle_period_fresh_ident_1 (timestamp) . await , } } else { break ; } }
            }
        }
        ...
        async fn handle_kickdown(&mut self, timestamp: i64, kickdown: Kickdown) {
            let (state, on_state, in_regulation_aux, state_update) = self.speed_limiter.step(
                self.context
        ...
        async fn handle_failure(&mut self, timestamp: i64, failure: Failure) {
            let (state, on_state, in_regulation_aux, state_update) = self.speed_limiter.step(
                self.context
        ...
        async fn handle_period_fresh_ident_1(&mut self, timestamp: i64) {
