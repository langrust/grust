...
pub mod runtime {
    ...
    impl Runtime {
        ...
        pub async fn run_loop(
            ...
            loop {
                tokio::select! { input = input . next () => if let Some (input) = input { match input { I :: speed (speed , instant) => { runtime . speed_limiter . handle_speed (instant , speed) . await ; } , I :: timer (T :: period_fresh_ident_1 , instant) => { runtime . speed_limiter . handle_period_fresh_ident_1 (instant) . await ; } , I :: kickdown (kickdown , instant) => { runtime . speed_limiter . handle_kickdown (instant , kickdown) . await ; } , I :: set_speed (set_speed , instant) => { runtime . speed_limiter . handle_set_speed (instant , set_speed) . await ; } , I :: vdc (vdc , instant) => { runtime . speed_limiter . handle_vdc (instant , vdc) . await ; } , I :: timer (T :: period_fresh_ident , instant) => { runtime . speed_limiter . handle_period_fresh_ident (instant) . await ; } , I :: vacuum_brake (vacuum_brake , instant) => { runtime . speed_limiter . handle_vacuum_brake (instant , vacuum_brake) . await ; } , I :: activation (activation , instant) => { runtime . speed_limiter . handle_activation (instant , activation) . await ; } , I :: failure (failure , instant) => { runtime . speed_limiter . handle_failure (instant , failure) . await ; } } } else { break ; } }
            }
        }
    }
    pub mod speed_limiter_service {
        ...
        impl SpeedLimiterService {
            ...
            pub async fn handle_period_fresh_ident(&mut self, instant: std::time::Instant) {
                let in_regulation = self.context.in_regulation_aux;
                {
                    let res = self
                        .output
