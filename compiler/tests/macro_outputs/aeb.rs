...
pub mod runtime {
    ...
    impl Runtime {
        ...
        pub async fn run_loop(
            ...
            loop {
                tokio::select! { input = input . next () => if let Some (input) = input { match input { I :: pedestrian_r (pedestrian_r , instant) => { runtime . aeb . handle_pedestrian_r (instant , pedestrian_r) . await ; } , I :: timer (T :: timeout_fresh_ident , instant) => { runtime . aeb . handle_timeout_fresh_ident (instant) . await ; } , I :: speed_km_h (speed_km_h , instant) => { runtime . aeb . handle_speed_km_h (instant , speed_km_h) . await ; } , I :: pedestrian_l (pedestrian_l , instant) => { runtime . aeb . handle_pedestrian_l (instant , pedestrian_l) . await ; } } } else { break ; } }
            }
        }
    }
    pub mod aeb_service {
