...
pub mod speed_limiter_service {
    ...
    impl SpeedLimiterService {
        ...
        pub async fn run_loop(
            ...
            loop {
                tokio::select! { input = input . next () => if let Some (input) = input { match input { I :: kickdown (kickdown , instant) => service . handle_kickdown (instant , kickdown) . await , I :: set_speed (set_speed , instant) => service . handle_set_speed (instant , set_speed) . await , I :: vacuum_brake (vacuum_brake , instant) => service . handle_vacuum_brake (instant , vacuum_brake) . await , I :: activation (activation , instant) => service . handle_activation (instant , activation) . await , I :: vdc (vdc , instant) => service . handle_vdc (instant , vdc) . await , I :: speed (speed , instant) => service . handle_speed (instant , speed) . await , I :: timer (T :: period_fresh_ident , instant) => service . handle_period_fresh_ident (instant) . await } } else { break ; } }
            }
        }
...
pub mod another_speed_limiter_service {
    use super::*;
    use futures::{sink::SinkExt, stream::StreamExt};
    use AnotherSpeedLimiterServiceInput as I;
    use AnotherSpeedLimiterServiceOutput as O;
    use AnotherSpeedLimiterServiceTimer as T;
    #[derive(Clone, Copy, PartialEq, Default)]
    pub struct Context {
        pub state_update: bool,
        pub v_update: bool,
    ...
    #[derive(PartialEq)]
    pub enum AnotherSpeedLimiterServiceTimer {
        period_fresh_ident,
    }
    impl timer_stream::Timing for AnotherSpeedLimiterServiceTimer {
        fn get_duration(&self) -> std::time::Duration {
            match self {
                T::period_fresh_ident => std::time::Duration::from_millis(10u64),
            }
    ...
    impl priority_stream::Reset for AnotherSpeedLimiterServiceInput {
        fn do_reset(&self) -> bool {
            match self {
                AnotherSpeedLimiterServiceInput::timer(timer, _) => {
                    timer_stream::Timing::do_reset(timer)
                }
                _ => false,
            }
        }
    }
    impl PartialEq for AnotherSpeedLimiterServiceInput {
        fn eq(&self, other: &Self) -> bool {
            match (self, other) {
                (I::activation(this, _), I::activation(other, _)) => this.eq(other),
                (I::set_speed(this, _), I::set_speed(other, _)) => this.eq(other),
    ...
    impl AnotherSpeedLimiterServiceInput {
        pub fn get_instant(&self) -> std::time::Instant {
            match self {
                I::activation(_, instant) => *instant,
                I::set_speed(_, instant) => *instant,
    ...
    pub enum AnotherSpeedLimiterServiceOutput {
        in_regulation(bool, std::time::Instant),
        v_set(f64, std::time::Instant),
    }
    ...
    impl AnotherSpeedLimiterService {
        ...
        pub async fn run_loop(
            ...
            loop {
                tokio::select! { input = input . next () => if let Some (input) = input { match input { I :: vdc (vdc , instant) => service . handle_vdc (instant , vdc) . await , I :: speed (speed , instant) => service . handle_speed (instant , speed) . await , I :: timer (T :: period_fresh_ident , instant) => service . handle_period_fresh_ident (instant) . await , I :: kickdown (kickdown , instant) => service . handle_kickdown (instant , kickdown) . await , I :: set_speed (set_speed , instant) => service . handle_set_speed (instant , set_speed) . await , I :: vacuum_brake (vacuum_brake , instant) => service . handle_vacuum_brake (instant , vacuum_brake) . await , I :: activation (activation , instant) => service . handle_activation (instant , activation) . await } } else { break ; } }
            }
        }
        async fn handle_vdc(&mut self, instant: std::time::Instant, vdc: VdcState) {
            self.context.vdc = vdc;
